...
from ..extract import flaggems_bridge
...
#: fallbacks too — the alternative is that `torch.compile(..., backend="toyisa")`
...
def _is_matmul_only_graph(targets: Sequence[str]) -> bool:
    ...
    if not meaningful:
        return False
    matmuls = [t for t in meaningful if t in MATMUL_TARGETS]
    elts = [t for t in meaningful if t in ELEMENTWISE_TARGETS]
    others = [t for t in meaningful if t not in MATMUL_TARGETS and t not in ELEMENTWISE_TARGETS]
...
def _multi_kernel_interpret(
    ...
    exposed as `toyisa_counters` and the reasons as plan fallbacks, so a graph
    ...
    class ToyISAInterpreter(Interpreter):
    ...
    interp = ToyISAInterpreter(graph)
    ...
    run.toyisa_counters = counters  # type: ignore[attr-defined]
    return run
...
def _match_recorded_shapes(
    ...
    for name, ttir_text in recorded_kernels().items():
        try:
...
def _pad_to_extent(kernel: CompiledKernel, name: str, tensor: torch.Tensor) -> torch.Tensor:
    ...
    pads: list[int] = []
    for got, want in zip(reversed(tuple(tensor.shape)), reversed(wanted)):
        if want < got:
            raise LoweringError(
...
def toyisa_backend(graph: Any, example_inputs: Sequence[torch.Tensor]) -> Callable[..., Any]:
    ...
    if not kernels:
        ...
        if _is_matmul_only_graph(targets):
            ...
            run.toyisa_multi_kernel = True  # type: ignore[attr-defined]
            return run
