...
toyisa1.yaml` declares `alignment_words: 4` under `data_model.memory_spaces`, and
...
#: `isa/schemas/toyisa1.yaml` → `data_model.memory_spaces[0].alignment_words`.
...
# `torch.cuda`-shaped module-level functions, so `torch.toyisa.allocate(...)`
