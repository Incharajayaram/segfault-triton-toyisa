"""The `DeviceInterface` seat: what makes `toyisa` a device PyTorch can name.
...
`get_interface_for_device("toyisa")` returns this class, and every part of
...
worse.** `torch.device("toyisa")`, `torch.toyisa.<anything>` and
`tensor.to("toyisa")` all require `torch.utils.rename_privateuse1_backend`, which
re-registers the *PrivateUse1* dispatcher key. Measured on this checkout
(torch 2.12.1+cpu):

    >>> torch.utils.rename_privateuse1_backend("toyisa")
    >>> torch.device("toyisa")            # works
    >>> torch.tensor([1.0]).to("toyisa")
    RuntimeError: PyTorch is not linked with support for toyisa devices

and — the finding that decided this design — the rename also makes
`torch.accelerator` treat `toyisa` as *the* accelerator, which breaks Dynamo for
**every** graph, toy device or not:

    torch._dynamo.exc.InternalTorchDynamoError: RuntimeError: PyTorch is not
    linked with support for toyisa devices
...
DATA_PLANE_GAP = (
    "`tensor.to('toyisa')` is not available: allocating a torch.Tensor on a "
    ...
    "Measured here: RuntimeError('PyTorch is not linked with support for toyisa "
...
RENAME_BLOCKED = (
    "`rename_privateuse1_backend('toyisa')` is deliberately not called. It would "
    "buy `torch.device('toyisa')` (and still not `tensor.to('toyisa')`, which needs "
    "a C++ allocator), but it makes torch.accelerator treat toyisa as the "
    "accelerator, so torch.accelerator.current_stream() raises "
    "'PyTorch is not linked with support for toyisa devices' inside "
...
class ToyIsaInterface(DeviceInterface):
    ...
    def current_device() -> int:
        return ToyIsaInterface._current_device
    ...
    def set_device(device: int | torch.device | str) -> None:
        ...
        ToyIsaInterface._current_device = index
    ...
    def get_device_properties(device: int | torch.device | str | None = None) -> Any:
        ...
        return ToyDeviceProperties(
            name="toyisa emulator (CPU numpy backend)",
            major=1,
            minor=0,
    ...
    class device:
        ...
        def __init__(self, device: Any) -> None:
            self.device = device
            self.previous = ToyIsaInterface._current_device

        def __enter__(self) -> ToyIsaInterface.device:
            ToyIsaInterface.set_device(self.device)
            return self

        def __exit__(self, *exc: object) -> bool:
            ToyIsaInterface._current_device = self.previous
            return False
    ...
    class Worker(DeviceInterface.Worker):
        ...
        @staticmethod
        def current_device() -> int:
            return ToyIsaInterface.current_device()

        @staticmethod
        def get_device_properties(device: Any = None) -> Any:
            return ToyIsaInterface.get_device_properties(device)

        @staticmethod
        def set_device(device: Any) -> None:
            ToyIsaInterface.set_device(device)
...
class _Exchange:
    """`with torch.toyisa.device(0):` — the device-context protocol."""

    def __init__(self, device: Any) -> None:
        self.target = device
        self.previous = device_module.DEVICE
        self.previous_index = ToyIsaInterface._current_device

    def __enter__(self) -> int:
        ToyIsaInterface.set_device(self.target)
        return ToyIsaInterface._current_device

    def __exit__(self, *exc: object) -> bool:
        ToyIsaInterface._current_device = self.previous_index
        del self.previous
        return False
...
def device_module_object() -> ModuleType:
    """The module `torch.toyisa` resolves to.

    Registered under the device name so `torch.toyisa.allocate(...)` works the way
    ...
    module.__doc__ = (
        ...
        "See triton_toyisa.torch_backend.device for the machine this drives."
    )
    for name in (
        "ALIGNMENT_WORDS",
        "DEVICE",
    ...
    module.is_available = ToyIsaInterface.is_available
    module.device_count = ToyIsaInterface.device_count
    module.current_device = ToyIsaInterface.current_device
    module.set_device = ToyIsaInterface.set_device
    module.get_device_properties = ToyIsaInterface.get_device_properties
...
def install() -> dict[str, Any]:
    """Make `toyisa` a device PyTorch can name. Idempotent, and C++-free.
    ...
    register_interface_for_device(DEVICE_NAME, ToyIsaInterface)
    _installed = True
    return {"interface_registered": True, "renamed": False, "cxx_required": list(UNAVAILABLE)}
