...
"""
Compile a PyTorch matmul through the ToyISA backend and dump the
generated ToyISA representation.

This intentionally does NOT use torch.device("toyisa").
The inputs remain ordinary CPU tensors; torch.compile() sends the
captured graph through the ToyISA backend and emulator.
"""
...
OUTPUT = Path("toyisa_generated.txt")
...
def main() -> None:
    ...
    if not HAS_CPP:
        print(
            ...
            "The ToyISA NumPy/reference emulator may be used instead."
        )
    ...
    print("\nCompiling with ToyISA...")
    compiled = torch.compile(kernel, backend="toyisa")

    print("Executing...")
    result = compiled(a, b)

    # ToyISA backend metadata exposed by your backend.
    plan = compiled.toyisa_plan

    print()
    print("========================================")
    print("ToyISA compilation plan")
    print("========================================")
    ...
    if not plan.fully_lowered:
        ...
        raise SystemExit(
            "\nERROR: graph was not fully lowered by ToyISA."
        )
    ...
    # ------------------------------------------------------------
    # Dump generated ToyISA representation.
    # ------------------------------------------------------------

    output: list[str] = []

    output.append("ToyISA generated code / IR")
    output.append("=" * 80)
    output.append("")
    ...
    if max_err > 1e-2:
        raise SystemExit(
            f"ERROR: ToyISA result differs from PyTorch by {max_err:.3e}"
        )
