#!/usr/bin/env python
"""Prove the toyisa Dynamo backend works, and that it runs on the C++ emulator.

Note what this does and does NOT demonstrate:

* `torch.device("toyisa")` / `tensor.to("toyisa")` are NOT supported here, by
  ...
  PrivateUse1 backend to "toyisa" breaks Dynamo tracing for every graph, toy
  or not. That boundary is a measured decision, not a bug.
* The real, working seam is the `toyisa_backend` registered under the name
  "toyisa" (what `torch.compile(fn, backend="toyisa")` looks up). It lowers
...
Why this calls `toyisa_backend` directly instead of going through
...
`toyisa_plan` from them onto the wrapper. `torch.fx.symbolic_trace` gives us
the same kind of GraphModule Dynamo would hand the backend, and calling
`toyisa_backend` on it directly gets us the exact `run` callable -- with
`toyisa_plan` attached -- so we can assert on it instead of guessing.
...
checkout (a 128x64 @ 64x128 tile), runs it through the backend, and checks:
  ...
  4. torch.compile(fn, backend="toyisa") also runs end-to-end and agrees.
"""
...
# importable with torch absent -- see triton_toyisa/__init__.py), so the
# @register_backend("toyisa") decorator in torch_backend/compiler.py only
# runs once this module is actually imported. Without this import,
# torch.compile(..., backend="toyisa") raises InvalidBackend.
...
def main() -> None:
    ...
    run = toyisa_backend(traced, [a, b])

    plan = run.toyisa_plan
    ...
    compiled = torch.compile(kernel, backend="toyisa")
    compiled_result = compiled(a, b)
    compiled_err = (compiled_result - expected).abs().max().item()
    print(f"torch.compile(backend='toyisa') max abs error: {compiled_err:.3e}")
    if compiled_err > 1e-2:
        raise SystemExit(f"torch.compile path diverged: {compiled_err:.3e}")

    print(
        "\nPASS: the toyisa backend lowered and executed a real matmul on the "
