...
def run_backend(fn, *args):
    call = seam.toyisa_backend(exported(fn, *args), args)
    result = call(*args)
...
def test_extract_matmul_pads_to_the_tile(shape: tuple[int, int, int]) -> None:
    ...
    assert tuple(p + q for p, q in zip(extracted.problem, extracted.pads)) == extracted.padded
...
def test_matmul_lowers_dynamically_and_matches_torch(shape: tuple[int, int, int]) -> None:
    ...
    provenance = [kernel.provenance for kernel in call.toyisa_plan.lowered]
...
def test_plan_records_why_each_stage_declined(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    joined = " ".join(record.reason for record in call.toyisa_plan.fallbacks)
...
def test_degrades_to_the_recorded_lowering_without_triton(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    call, result = run_backend(lambda x, y: x @ y, a, b)
    assert [kernel.provenance for kernel in call.toyisa_plan.lowered] == ["recorded"]
    ...
    notes = {note.stage: note.reason for note in call.toyisa_plan.notes}
    assert "unavailable" in notes.get("extract", ""), notes
    assert call.toyisa_plan.fallbacks == [], "nothing fell back to eager here"
    assert call.toyisa_plan.fully_lowered is True
...
def test_no_triton_also_records_the_bridge_reason(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    reasons = {record.stage: record.reason for record in call.toyisa_plan.fallbacks}
...
def test_eager_node_is_named_and_not_hidden() -> None:
    ...
    assert call.toyisa_plan.fully_lowered is False
    eager = [r for r in call.toyisa_plan.fallbacks if r.stage == "node"]
    reasons = " ".join(record.reason for record in call.toyisa_plan.fallbacks)
...
def test_undeclared_input_dtype_falls_back_instead_of_raising(dtype: torch.dtype) -> None:
    ...
    assert call.toyisa_plan.lowered == []
    assert call.toyisa_plan.fallbacks, "the refusal has to be recorded, not just survived"
    record = call.toyisa_plan.fallbacks[0]
...
def test_tf32_declared_precision_survives_a_rounded_dtype_band() -> None:
    ...
    assert [k.provenance for k in call.toyisa_plan.lowered] == ["dynamic"]
    assert call.toyisa_plan.fallbacks == []
...
def test_plan_json_reports_provenance_and_counters() -> None:
    ...
    payload = call.toyisa_plan.to_json()
