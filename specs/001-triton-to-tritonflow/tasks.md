---
description: "Task list for 001-triton-to-tritonflow"
---

# Tasks: Triton-IR-to-Declarative-Toy-ISA

**Input**: Design documents from `specs/001-triton-to-tritonflow/`
...
Single project. Source: `src/tritonflow/`. Tests: `tests/`. Specs: `specs/001-triton-to-tritonflow/`.
...
- [ ] T001 Create the package skeleton (`src/tritonflow/{ttir,canon,recognize,idioms,isa,emit,emu,torch_backend,report,harness}/__init__.py`) with an `__init__.py` that imports nothing heavy
...
- [ ] T004 [P] Add a runtime-import guard test in `tests/unit/test_no_triton_runtime.py` asserting `import tritonflow` succeeds with Triton absent from `sys.modules`
...
- [ ] T013 Implement `torch_backend/device_interface.py` (`TritonFlowInterface`, `register_interface`) with the slot inventory recorded in `tests/contract/test_device_interface.py`
...
| FR-003 dynamic LLVM/MLIR & JIT integration enabled | `src/tritonflow/extract/dynamic_extract.py`, `src/tritonflow/extract/flaggems_bridge.py`, `tests/unit/test_no_triton_runtime.py` (T004), `verify/verify_mlir_bindings.py` |
