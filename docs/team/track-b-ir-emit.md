...
```
src/triton_tritonflow/ttir/ssa.py               Module, Function, Operation, Region, Block, SsaValue, TypeExpr, Loc, Attr
src/triton_tritonflow/ttir/types.py             type text -> TypeExpr (shape, dtype, address space)
src/triton_tritonflow/ttir/to_ir.py             build_ir(RawModule) -> ParseResult
src/triton_tritonflow/ttir/graph.py             build_def_use, walk_region, topo_within_region, iter_loops
src/triton_tritonflow/canon/canonicalize.py     idempotent only
src/triton_tritonflow/emit/ir.py                Instr, Loop, Program, Operand, UnsupportedMarker
src/triton_tritonflow/emit/assemble.py          assemble, order_regions, emit_instr, check_constraint
src/triton_tritonflow/emit/disasm.py            serialize, deserialize, disassemble
src/triton_tritonflow/cli.py                    compile/report/transfer subcommands
src/triton_tritonflow/report/transfer.py        the per-stage transfer table
