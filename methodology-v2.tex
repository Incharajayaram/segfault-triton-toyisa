...
\code{tensorlift.mac} and \code{taidl.*} annotations (e.g.\ \code{tritonflow.mac},
\code{tritonflow.dma\_role}), so that assembly is a pure consumer of annotations.
...
Any operation reaching assembly without a \code{tritonflow.*} annotation is emitted as an explicit
...
def tritonflow(gm, example_inputs):
...
register_backend(name="tritonflow")(tritonflow)
...
\code{tritonflow.*} annotation''. \textbf{This metric is gameable and semantically weak:} one unannotated
...
\item \textbf{Module and contract coverage is checked, not trusted}: every module under
      \path{src/triton_tritonflow} must have \path{tests/unit/test_<module>.py}, and every contract file must have
